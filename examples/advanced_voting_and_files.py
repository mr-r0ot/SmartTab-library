...
Demonstrates:
  ...
  - ensemble="auto" (the decision engine: tunes CatBoost + LightGBM, and only
    pays for a full CatBoost+LightGBM+XGBoost voting/stacking ensemble when
    the two are close enough that combining them plausibly helps)
...
if __name__ == "__main__":
    ...
    model = smarttab.fit(csv_path, target="churned", ensemble="auto", n_trials=10, cv=3, report=False)
