...
📘 **[documents.md](SmartTab-3.1.0/documents.md)** — the full guide: install → first model →
real projects → every parameter, from a beginner's first `fit()` call to
production-grade tuning.

📘 **[full_documents.md](SmartTab-3.1.0/full_documents.md)** — the more full guide

⚙️ **[HowItWorks.md](SmartTab-3.1.0/HowItWorks.md)** — what actually happens inside `fit()`
...
model = smarttab.fit(
  ...
  #ensemble='auto',
  #model='catboot'
)   # analyzes, cleans, tunes, trains, evaluates
